...
contract SupportChildren is Ownable, ISupportChildren {
    ...
    constructor() {
        ...
        allowedCurrencies[0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984] = true;
        allowedCurrencies[0xc7AD46e0b8a400Bb3C915120d284AafbA8fc4735] = true; // allow to recieve DAI
    }
    ...
    mapping(uint => uint) public farmingSupply;
    ...
    function updateERC20Contributions(uint _campaignId, uint donation, address _from) private {
         ...
         emit Donation(campaigns[_campaignId].name, _from, _campaignId, donation, campaigns[_campaignId].cryptocurrency, campaigns[_campaignId].beneficiary);
    ...
    function createCampaign(string memory name, address payable _beneficiary, uint _endTimestamp, uint _goal,  address _cryptocurrency,string calldata uri) override external conditions(msg.sender, _cryptocurrency, _endTimestamp) {
        ...
        Campaign memory campaign = Campaign(name, msg.sender, _endTimestamp, beneficiary, false, _goal, _cryptocurrency, uri);
    ...
    function donate(uint _campaignId, address _token, uint _amount) override external valid(_campaignId, _amount) {
     ...
     // 3. update contributions mappings
            ...
            if(_token == campaign.cryptocurrency){
                ...
                updateERC20Contributions(_campaignId, donation, msg.sender);
    ...
    function finalPayment(uint _campaignId) override public payable restricted(_campaignId) {
         ...
         if(campaigns[_campaignId].cryptocurrency == address(0)){
             payable(campaigns[_campaignId].beneficiary).transfer(campaign_contributions[_campaignId] - farmingSupply[_campaignId]);
             campaigns[_campaignId].completed = true;
             raisedAmount[address(0)] = address(this).balance;
         }
         else{
             IERC20(campaigns[_campaignId].cryptocurrency).transfer(campaigns[_campaignId].beneficiary,campaign_contributions[_campaignId] - farmingSupply[_campaignId]);
             campaigns[_campaignId].completed = true;
             raisedAmount[campaigns[_campaignId].cryptocurrency] = IERC20(campaigns[_campaignId].cryptocurrency).balanceOf(address(this));
            }
    }
    ...
    function getFundsForFarming(uint _campaignId) external payable{
        
        require(isOrganization[msg.sender] == true, "SupportChildren::getFundsForFarming: Only organization can get funds for farming");
        uint current_amount = campaign_contributions[_campaignId] - farmingSupply[_campaignId];
        
        if(campaigns[_campaignId].cryptocurrency == address(0)){
             payable(campaigns[_campaignId].beneficiary).transfer(current_amount);
             farmingSupply[_campaignId] += current_amount;
             totalSupply += current_amount;
             raisedAmount[address(0)] = address(this).balance;
         }
         else{
             
             IERC20(campaigns[_campaignId].cryptocurrency).transfer(campaigns[_campaignId].beneficiary,current_amount);
             farmingSupply[_campaignId] += current_amount;
             totalSupply += current_amount;
             raisedAmount[campaigns[_campaignId].cryptocurrency] = IERC20(campaigns[_campaignId].cryptocurrency).balanceOf(address(this));
         }
